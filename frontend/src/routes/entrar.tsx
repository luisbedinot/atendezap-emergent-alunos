...
function EntrarPage() {
  ...
  const [lockLogin, setLockLogin] = useState(search.modo === "login");
                ...
                <form onSubmit={handleSubmit} className="space-y-4">
                  <div className="space-y-1.5">
                    ...
                    <Input
                      ...
                      type="email"
                      value={email}
                      onChange={(e) => { setEmail(e.target.value); if (needsPassword && !lockLogin) setNeedsPassword(false); }}
                      required
                      autoFocus
                  ...
                  <div className="text-center pt-1">
                    <button
                      ...
                      onClick={() => {
                        if (needsPassword) { setNeedsPassword(false); setLockLogin(false); }
                        else { setNeedsPassword(true); setLockLogin(true); }
                      }}
                      className="text-[12.5px] font-medium text-[color:var(--brand-text)] hover:underline underline-offset-2"
                    >
                      {needsPassword ? "Não tem conta? Criar conta grátis" : "Já tenho conta — Entrar"}
                    </button>
                  </div>
                </form>
              </div>
